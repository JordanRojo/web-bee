...
const allSampleReportsData = {
    ...
    dailySensorData: [
        { name: 'Colmena 001', temperature: 34.8, humidity: 61.2, weight: 47.5, date: '2025-07-24', id: 'h1' },
        { name: 'Colmena 002', temperature: 39.1, humidity: 78.0, weight: 32.5, date: '2025-07-24', id: 'h2' },
        { name: 'Colmena 003', temperature: 36.5, humidity: 68.0, weight: 40.0, date: '2025-07-24', id: 'h3' },
        { name: 'Colmena 004', temperature: 33.9, humidity: 58.5, weight: 51.0, date: '2025-07-24', id: 'h4' },
        { name: 'Colmena 005', temperature: 34.2, humidity: 60.1, weight: 48.2, date: '2025-07-24', id: 'h5' },
        { name: 'Colmena 006', temperature: 37.0, humidity: 70.0, weight: 30.1, date: '2025-07-24', id: 'h6' },
        { name: 'Colmena 007', temperature: 35.5, humidity: 63.0, weight: 45.0, date: '2025-07-24', id: 'h7' },
        { name: 'Colmena 008', temperature: 38.0, humidity: 80.0, weight: 28.0, date: '2025-07-24', id: 'h8' },
        { name: 'Colmena 001', temperature: 34.0, humidity: 60.0, weight: 47.0, date: '2025-07-23', id: 'h1' },
        { name: 'Colmena 002', temperature: 38.5, humidity: 77.0, weight: 32.0, date: '2025-07-23', id: 'h2' },
    ],
    alertHistory: [
...
const ReportsScreen = () => {
    ...
    const getFilteredReportData = useMemo(() => {
        return (date, hiveId) => {
            const filteredDailySensorData = allSampleReportsData.dailySensorData.filter(d => d.date === date);
            const filteredAlertHistory = allSampleReportsData.alertHistory.filter(a => a.date === date);

            if (hiveId !== 'all') {
                const singleHiveData = filteredDailySensorData.filter(d => d.id === hiveId);
                const singleHiveAlerts = filteredAlertHistory.filter(a => a.hiveId === hiveId);
                ...
                if (singleHiveData.length > 0) {
                    ...
                    else if (hive.weight >= 30 && hive.weight <= 40) newOverview.alertHives = 1;
                ...
                if (filteredDailySensorData.length > 0) {
                    ...
                    newOverview.healthyHives = filteredDailySensorData.filter(h => h.weight > 40).length;
                    newOverview.alertHives = filteredDailySensorData.filter(h => h.weight >= 30 && h.weight <= 40).length;
                    newOverview.criticalHives = filteredDailySensorData.filter(h => h.weight < 30).length;
                } else {
    ...
    const uniqueHives = useMemo(() => {
        const hives = new Set();
        allSampleReportsData.dailySensorData.forEach(d => hives.add({ id: d.id, name: d.name }));
