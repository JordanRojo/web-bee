...
const DashboardScreen = () => {
  ...
  const allAlertsData = useMemo(() => {
    return colmenas
      .map((colmena) => {
        let alerts = [];
        if (colmena.temperatura < 33 || colmena.temperatura > 35) {
          alerts.push({
            type: "Temperatura fuera del rango esperado",
            value: colmena.temperatura + "°C",
            severity: "High",
          });
        }
        if (colmena.humedad < 40 || colmena.humedad > 70) {
          alerts.push({
  ...
  const okColonies = colmenas.filter(
    (c) =>
          ...
          >
            ...
            <div className="average-metrics-display">
              <div className="avg-item">
                <MdOutlineThermostat /> **Temp. Promedio:**{" "}
                <span>{avgTemp.toFixed(1)}°C</span>
              </div>
              <div className="avg-item">
                <MdOutlineWaterDrop /> **Hum. Promedio:**{" "}
                <span>{avgHum.toFixed(1)}%</span>
              </div>
              <div className="avg-item">
                <MdOutlineScale /> **Peso Promedio:**{" "}
                <span>{avgWeight.toFixed(1)} kg</span>
              </div>
            </div>
