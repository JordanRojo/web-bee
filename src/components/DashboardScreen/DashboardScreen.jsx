...
const useSimulatedHistoricalData = (avgMetrics) => {
    ...
    return useMemo(() => {
        ...
        for (let i = 23; i >= 0; i--) {
            ...
            const temp = baseTemp + Math.sin((hour / 24) * 2 * Math.PI) * 1.5 + (Math.random() - 0.5) * 0.5;
            const hum = baseHum + Math.cos((hour / 24) * 2 * Math.PI) * 5.0 + (Math.random() - 0.5) * 2.0;
            const weight = baseWeight + Math.sin((hour / 24) * 2 * Math.PI + Math.PI / 2) * 0.5 + (Math.random() - 0.5) * 0.3;
            
            tempHistory.push({ time: timeLabel, Temperatura: parseFloat(temp.toFixed(1)) });
            humHistory.push({ time: timeLabel, Humedad: parseFloat(hum.toFixed(1)) });
            weightHistory.push({ time: timeLabel, Peso: parseFloat(weight.toFixed(1)) });
        }
    ...
    return (
        <div className="trend-selector-bar-floating">
            {trendOptions.map(option => (
                <button 
                    key={option.key}
                    className={`trend-button ${activeTrend === option.key ? 'active' : ''}`} 
...
const SingleMetricTrendChart = ({ data, dataKey, name, unit, strokeColor, domain }) => (
    <div className="trend-chart-container">
        ...
        <p className="chart-subtitle">
            Evolución de la métrica promedio.
        </p>
